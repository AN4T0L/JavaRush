...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        AtomicInteger TGcounter = new AtomicInteger(0);
        static AtomicInteger TGcounter1 = new AtomicInteger(0);
        AtomicInteger TGcounter2 = new AtomicInteger(0);

        public AmigoThreadFactory(){
            TGcounter2.set(TGcounter1.incrementAndGet());
        }

        @Override
        public Thread newThread(Runnable r)
        {
            TGcounter.addAndGet(1);
            Thread thread = new Thread(r);


            thread.setName(thread.getThreadGroup().getName()+"-pool-"+TGcounter2+"-thread-"+TGcounter);
